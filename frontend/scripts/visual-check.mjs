...
for (const viewport of [{name:'desktop',width:1440,height:900},{name:'mobile-320',width:320,height:700}]) {
  ...
  await page.screenshot({path:`visual-${viewport.name}-forgot-password.png`,fullPage:true})
  const recoveryOverflow = await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth)

  await page.route('**/api/me/', route => route.fulfill({json:{id:1,email:'alice@example.test',public_id:'alice',display_name:'Alice Martin',bio:'Disponible le soir',email_verified:true,email_verified_at:new Date().toISOString()}}))
  await page.route('**/api/sessions/', route => route.fulfill({json:[{id:'11111111-1111-4111-8111-111111111111',description:'Chrome sur Windows (indication)',ip_address:'192.168.1.13',created_at:new Date().toISOString(),last_activity_at:new Date().toISOString(),current:true},{id:'22222222-2222-4222-8222-222222222222',description:'Chrome sur Android (indication)',ip_address:'192.168.1.20',created_at:new Date().toISOString(),last_activity_at:new Date().toISOString(),current:false}]}))
  await page.route('**/api/blocks/', route => route.fulfill({json:[{id:1,user:{id:3,public_id:'charlie',display_name:'Charlie Test',bio:''},created_at:new Date().toISOString()}]}))
  await page.route('**/api/conversations/', route => route.fulfill({json:[{id:conversationId,other_user:{id:2,public_id:'bob',display_name:'Bob Durand',bio:''},last_message:{id:2,server_sequence:2,conversation_id:conversationId,author_id:2,client_id:'22222222-2222-4222-8222-222222222222',content:'On se retrouve à 18 h ?',created_at:new Date().toISOString()},blocked_by_me:false,updated_at:new Date().toISOString(),created_at:new Date().toISOString()}]}))
  await page.route(`**/api/conversations/${conversationId}/`, route => route.fulfill({json:{id:conversationId,other_user:{id:2,public_id:'bob',display_name:'Bob Durand',bio:''},last_message:null,blocked_by_me:false,updated_at:new Date().toISOString(),created_at:new Date().toISOString()}}))
  ...
  await page.screenshot({path:`visual-${viewport.name}-settings.png`,fullPage:true})
  const settingsOverflow = await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth)
  results.push({viewport:viewport.name,loginOverflow,recoveryOverflow,chatOverflow,settingsOverflow,composerVisible})
  await page.close()
}
