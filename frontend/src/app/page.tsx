...
function Dashboard() {
  ...
  return <main className="main"><div className="shell"><div className="page-heading"><h1>Une partie, entre amis.</h1><p className="subtitle">Installez-vous, la table est prête.</p></div><div className="dashboard-grid"><div><section className="card hero"><p className="eyebrow">À vous de jouer</p><h2>Quel sera votre prochain coup ?</h2><div className="hero-actions"><Link className="btn primary" href="/play/ai">Jouer contre l’IA</Link><Link className="btn" href="/friends">Inviter un ami</Link></div><span className="hero-piece" aria-hidden>♞</span></section><h2 className="section-title">Continuer à apprendre</h2><div className="lesson-cards">{lessons.slice(0, 2).map((lesson) => <article className="card lesson-card" key={lesson.id}><span className="eyebrow">{lesson.difficulty}</span><h3>{lesson.title}</h3><p>{lesson.objective}</p><Link className="btn" href={`/lessons/${lesson.id}`}>{lesson.progress?.completed ? "Revoir" : "Commencer"}</Link></article>)}</div></div><aside className="card friends-panel"><h2>Vos amis</h2><FriendsList friends={friends} /><div className="message-preview"><span className="eyebrow">Messages</span><p>{friends.length ? `Écrivez à ${friends[0].display_name}` : "Ajoutez un ami pour discuter."}</p><Link href="/messages" style={{ color: "var(--green)", fontWeight: 600 }}>Voir les messages →</Link></div></aside></div></div></main>;
}
